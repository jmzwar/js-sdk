...
class Spot {
  ...
  async getSettlementStrategy(settlementStrategyId, marketId, marketName) {
    ...
    const settlementStrategy = await this.marketProxy.getSettlementStrategy(
      resolvedMarketId,
  ...
  async commitOrder(side, size, settlementStrategyId = 2, marketId, marketName, submit) {
    ...
    if (submit) {
      ...
      this.logger.info(
        `Committing ${side} order of size ${weiToEther(
          sizeWei
        )} (${size}) to ${resolvedMarketName} (id: ${resolvedMarketId})`
      );
