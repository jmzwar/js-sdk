...
async function testPerpsAccountFetch() {
  try {
    ...
    logger.info(`Address: ${snx.address} - accounts: ${accountIds.length} - account_ids: ${JSON.stringify(accountIds)}`);
...
async function testPerpsMarketSummary() {
  ...
  async function testPerpsSettlementStrategy() {
    try {
      ...
      logger.info(`id: ${TEST_SETTLEMENT_STRATEGY_ID} - settlement strategy: ${JSON.stringify(settlementStrategy)}`);
