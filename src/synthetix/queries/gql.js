...
const transfers = gql`
  query ($last_id: ID!, $min_timestamp: BigInt = 0, $max_timestamp: BigInt!) {
    futuresMarginTransfers(
      where: { id_gt: $last_id, timestamp_gt: $min_timestamp, timestamp_lt: $max_timestamp }
      first: 1000
    ) {
      id
      account
...
const transfersAccount = gql`
  query ($last_id: ID!, $account: Bytes!, $min_timestamp: BigInt = 0, $max_timestamp: BigInt!) {
    futuresMarginTransfers(
      where: {
        id_gt: $last_id
        account: $account
...
const positionsMarket = gql`
  query ($last_id: ID!, $market_keys: [Bytes!], $is_open: [Boolean!]) {
    futuresPositions(
      where: { id_gt: $last_id, marketKey_in: $market_keys, isOpen_in: $is_open }
      first: 1000
    ) {
      id
      lastTxHash
...
const positionsAccount = gql`
  query ($last_id: ID!, $account: Bytes!, $is_open: [Boolean!]) {
    futuresPositions(
      where: { id_gt: $last_id, account: $account, isOpen_in: $is_open }
      first: 1000
    ) {
      id
      lastTxHash
