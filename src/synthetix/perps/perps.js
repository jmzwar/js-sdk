...
import { COLLATERALS_BY_ID, COLLATERALS_BY_NAME, PERPS_MARKETS_BY_ID, PERPS_MARKETS_BY_NAME } from './constants.js';
...
class Perps {
  ...
  async getOrder(accountId = null, fetchSettlementStrategy = true) {
      ...
      const [marketId, , sizeDelta, settlementStrategyId, acceptablePrice, trackingCode, referrer] = request;
      ...
      if (fetchSettlementStrategy) {
          const settlementStrategy = await this.getSettlementStrategy(settlementStrategyId, { marketId });
  ...
  async getMarketSummaries(marketIds = []) {
      ...
      const markets = await multicallErc7412(
          this.snx,
          this.marketProxy,
          'getMarketSummary',
          inputs,
...
async getCanLiquidate(accountId = null) {
    ...
    const canLiquidate = await callErc7412(
        this.snx,
        this.marketProxy,
        'canLiquidate',
        accountId,
...
async modifyCollateral(amount, { marketId, marketName, accountId = null, submit = false }) {
  const { marketId: resolvedMarketId, marketName: resolvedMarketName } = await this._resolveMarket(
      marketId,
      marketName,
      true
  );
  ...
  const marketProxy = this.marketProxy;
  const txParams = await writeErc7412(
      this.snx,
      this.marketProxy,
      'modifyCollateral',
      [accountId, resolvedMarketId, ethers.utils.parseUnits(amount, 'ether')]
  );
...
async settlePythOrder({
  ...
  const accountBytes = ethers.utils.hexZeroPad(ethers.BigNumber.from(accountId).toHexString(), 32);
  const marketBytes = ethers.utils.hexZeroPad(ethers.BigNumber.from(order.market_id).toHexString(), 32);

  // Concatenate the bytes and convert to hex
  const extraData = `0x${ethers.utils.hexlify(ethers.utils.concat([accountBytes, marketBytes])).slice(2)}`;
